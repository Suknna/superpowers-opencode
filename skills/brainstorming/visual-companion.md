...
The command returns JSON with `url`, `screen_dir`, and `state_dir`. Save those paths for the session. With `--open`, the browser opens to the first screen on its own once you push it; in headless or remote setups that can't auto-open, share the `url` field (with its `?key=…`) as a fallback. If the command must remain attached for the server to survive in the current OpenCode environment, run it in the background using OpenCode's shell execution support and read `$STATE_DIR/server-info` on the next turn.
...
1. **Check server is alive**, then **write HTML** to a new file in `screen_dir`:
   ...
   - Use OpenCode's file editing tools — **never use cat/heredoc** (dumps noise into terminal)
